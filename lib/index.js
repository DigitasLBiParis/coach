...
// Handle more command-line options
if (typeof argv.coach !== 'undefined') {
  DEFAULT_OPTIONS.coach = merge({}, DEFAULT_OPTIONS.coach, argv.coach);
}

// Handle config file
if (typeof argv.config === 'string') {
  DEFAULT_OPTIONS.coach = merge(
    {},
    DEFAULT_OPTIONS.coach,
    /^\//.test(argv.config)
      ? require(argv.config).coach
      : require(path.join(process.env.PWD, argv.config)).coach
  );
}
